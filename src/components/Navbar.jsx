                  ...
                  return (
                    <>
                      <span
                        ...
                        style={{
                          background:
                            "linear-gradient(180deg, #ffffff 0%, var(--title) 35%, #ffffff 55%, var(--title) 100%)",
                      ...
                      {rest && (
                        <span
                          className="font-semibold text-sm sm:text-base md:text-lg ml-1.5 align-middle"
                          style={{
                            background:
                              "linear-gradient(180deg, #ffffff 0%, var(--subtitle) 40%, #ffffff 60%, var(--subtitle) 100%)",
                            WebkitBackgroundClip: "text",
                            WebkitTextFillColor: "transparent",
                  ...
                  <li key={cat.name} className="flex" onMouseEnter={() => (hasMenu ? openMenu(cat) : scheduleClose())} onMouseLeave={scheduleClose}>
                    ...
                    {hasMenu && isOpen && (
                      <div className="absolute left-0 right-0 top-full z-50 pt-3 -mt-3" onMouseEnter={() => openMenu(cat)} onMouseLeave={scheduleClose}>
                        <div className="shadow-2xl border-t-[3px] rounded-b-lg overflow-hidden" style={{ borderTopColor: cat.accent, backgroundColor: "var(--secondary)" }}>
                          ...
                          <div className="grid grid-cols-2 px-8 py-7 gap-8">
                                ...
                                <div key={group.title}>
                                  ...
                                  <ul className="space-y-1.5">
                                    {group.items.map((item) => (
                                      <li key={item}>
                                        <button className="text-[13px] transition-all hover:translate-x-1" style={{ color: "var(--subtitle)" }} onMouseEnter={(e) => e.currentTarget.style.color = "var(--details)"} onMouseLeave={(e) => e.currentTarget.style.color = "var(--subtitle)"} onClick={() => goSub(cat, item)}>{item}</button>
                                      </li>
                                    ))}
                                  </ul>
                                </div>
                            ...
                            <div className="border-l pl-8" style={{ borderColor: "var(--border)" }}>
                              <h4 className="text-[11px] font-bold uppercase tracking-widest mb-3" style={{ color: "var(--subtitle)" }}>New Arrivals</h4>
                                ...
                                {(featuredByCat[catSlug(cat)] && featuredByCat[catSlug(cat)].length > 0)
                                  ? featuredByCat[catSlug(cat)].slice(0, 10).map((p) => (
                                    <button key={p.id ?? p.slug} onClick={() => go(`/product/${p.slug}`)} className="text-left group/card">
                                      <div className="aspect-[3/4] rounded overflow-hidden" style={{ backgroundColor: "#0F0F0F" }}>
                                      ...
                                      <p className="mt-1 text-[10px] leading-tight truncate" style={{ color: "var(--details)" }}>{p.name}</p>
                                      <p className="text-[10px] font-bold" style={{ color: "var(--title)" }}>{taka(p.price)}</p>
                                    </button>
                                  ))
                                  : Array.from({ length: 10 }).map((_, i) => (
                                    <div key={i} className="animate-pulse">
                                      <div className="aspect-[3/4] rounded" style={{ backgroundColor: "rgba(255,255,255,.05)" }} />
                                      <div className="mt-1 h-2 rounded w-3/4" style={{ backgroundColor: "rgba(255,255,255,.05)" }} />
                                      <div className="mt-1 h-2 rounded w-1/2" style={{ backgroundColor: "rgba(255,255,255,.05)" }} />
                                    </div>
                                  ))}
                              </div>
                            </div>
                          ...
                          <div className="border-t px-8 py-3" style={{ borderColor: "var(--border)", backgroundColor: "var(--primary)" }}>
            ...
            <div className="hidden md:flex items-center gap-4 lg:gap-5 shrink-0">
              ...
              <div className="relative group">
                ...
                <div className="absolute right-0 top-full z-50 hidden group-hover:block w-60 pt-2">
                  <div className="shadow-xl border border-gray-100 rounded-md overflow-hidden" style={{ backgroundColor: "var(--secondary)" }}>
                    <div className="px-4 py-3">
                      {user ? (
                        ...
                        <>
                          <p className="text-sm" style={{ color: "var(--subtitle)" }}>Welcome</p>
                    ...
                    <div className="border-t py-1" style={{ borderColor: "var(--border)", backgroundColor: "var(--primary)", color: "var(--title)", fontSize: "13px" }}>
                      <ProfileItem
          ...
          <>
            ...
            <div className="fixed top-0 right-0 h-full w-[75%] max-w-[360px] flex flex-col px-4 pt-16 pb-10 xl:hidden z-[100] overflow-y-auto shadow-2xl animate-[slideIn_.25s_ease-out]" style={{ backgroundColor: "var(--secondary)" }}>
              ...
              {categories.map((cat) => {
                ...
                return (
                  <div key={cat.name} className="w-full">
                    <button className="text-gray-800 text-base font-semibold py-3 w-full text-left border-b flex justify-between items-center" onClick={() => (hasMenu ? setOpenDropdown(openDropdown === cat.name ? null : cat.name) : goCategory(cat))} style={openDropdown === cat.name ? { color: cat.accent } : undefined, { backgroundColor: "var(--primary)", color: "var(--title)" ,paddingLeft: "0.75rem", paddingRight: "0.75rem"}}>
                      {cat.name}
                      {hasMenu && <span className="ml-4 text-gray-400">{openDropdown === cat.name ? "-" : "+"}</span>}
                    </button>
                    {hasMenu && openDropdown === cat.name && (
                      <div className="py-2" style={{ backgroundColor: "var(--primary)", color: "var(--title)" }}>
                        {cat.groups.map((group) => (
                          <div key={group.title} className="px-4 py-1">
                            <p className="text-[11px] font-bold uppercase tracking-widest mb-1" style={{ color: "var(--title)" }}>{group.title}</p>
                            {group.items.map((item) => (
                              <button key={item} className="text-sm py-1.5 block w-full text-left" style={{ color: "var(--subtitle)" }} onClick={() => goSub(cat, item)}>{item}</button>
                            ))}
                          </div>
                        ))}
              ...
              <div className="mt-6 border-t border-gray-200 pt-4 space-y-1">
                ...
                <button onClick={() => go("/account/orders")} className="block py-2 text-sm" style={{ color: "var(--subtitle)", hover: { backgroundColor: "var(--button)" } }}>My Orders</button>
                <button onClick={() => go("/track-order")} className="block py-2 text-sm" style={{ color: "var(--subtitle)", hover: { backgroundColor: "var(--button)" } }}>Track Order</button>
                {user && <button onClick={onLogout} className="block py-2 text-sm" style={{ color: "var(--subtitle)", hover: { backgroundColor: "var(--button)" } }}>Logout</button>}
              </div>
            </div>
...
function MobileAction({ icon: Icon, label, onClick, badge = 0, BadgeCmp }) {
  return (
    <button onClick={onClick} className="relative flex flex-col items-center gap-0.5" style={{ color: "var(--title)" }}>
