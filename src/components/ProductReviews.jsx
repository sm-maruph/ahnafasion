...
export default function ProductReviews({ productId }) {
  ...
  return (
    <section className="mt-12 border-t border-gray-100 pt-8">
      <h2 className="text-xl font-bold" style={{ color: "var(--title)" }}>
        Ratings & Reviews
        ...
        <div className="lg:col-span-1">
          <div className="flex items-center gap-4">
            <div className="text-center">
              <p className="text-4xl font-extrabold" style={{ color: "var(--title)" }}>{summary.avg || 0}</p>
              <Stars value={summary.avg} />
              <p className="text-xs" style={{ color: "var(--subtitle)" }}>{summary.count} review{summary.count === 1 ? "" : "s"}</p>
            </div>
            <div className="flex-1 space-y-1">
              {[5, 4, 3, 2, 1].map((n) => (
                <div key={n} className="flex items-center gap-2 text-xs" style={{ color: "var(--subtitle)" }}>
          ...
          <div className="mt-6 rounded-xl border border-gray-100 p-4">
            <p className="text-sm font-bold" style={{ color: "var(--details)" }}>Write a review</p>
              ...
              {items.map((r) => (
                <div key={r.id} className="border-b border-gray-50 pb-4 last:border-0">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-semibold" style={{ color: "var(--subtitle)" }}>{r.name}{r.isGuest && <span className="ml-1 text-[10px] font-medium" style={{ color: "var(--subtitle)" }}>(guest)</span>}</p>
                      <div className="flex items-center gap-2"><Stars value={r.rating} size={15} /><span className="text-xs" style={{ color: "var(--subtitle)" }}>{fmtDate(r.date)}</span></div>
                    </div>
                  </div>
